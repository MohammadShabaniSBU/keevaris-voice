...
| Slow `ask()` must not skip the typing clip | `filler-sound-survives-slow-delegation.json` |
| Lost filler `AgentAudioDone` is bounded from answer-ready | `lost-filler-audio-done-bounded-by-hold.json` |
